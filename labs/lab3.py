...
class CFGTrainer(Trainer):
    ...
    def get_train_loss(self, batch_size: int) -> torch.Tensor:
        ...
        u_model = self.model.forward(x, t, y)
        return nn.MSELoss()(u_model, u_ref)
...
# **Your answer**: 
#
# **FourierEncoder** transforms scalar time values into sinusoidal embeddings that capture temporal information across different frequencies. It takes time tensors of shape `(bs, 1, 1, 1)` and outputs embeddings of shape `(bs, dim)` by computing sine and cosine functions of the time multiplied by learnable frequency weights, enabling the model to distinguish between different timesteps in the flow matching process.
#
# **ResidualLayer** serves as the fundamental processing block that combines image features with time and label conditioning while maintaining spatial resolution. It processes image features `(bs, c, h, w)` along with time and label embeddings through two convolutional blocks with injected conditioning information, using residual connections to enable stable training and allow the model to learn incremental transformations rather than complete reconstructions.
#
# **Encoder** progressively downsamples the input while extracting hierarchical features at multiple scales. It takes images of shape `(bs, c_in, h, w)` and produces downsampled features `(bs, c_out, h//2, w//2)` by applying multiple residual layers followed by a strided convolution, capturing both fine-grained local details and broader spatial patterns needed for image generation.
#
# **Decoder** reconstructs spatial resolution by upsampling features while incorporating skip connections from the encoder path. It transforms features from `(bs, c_in, h, w)` to `(bs, c_out, 2h, 2w)` through bilinear upsampling followed by residual processing, enabling the model to recover fine details while maintaining global coherence in the generated images.
#
# **Midcoder** processes features at the bottleneck of the U-Net where spatial resolution is lowest but feature depth is highest. It applies multiple residual layers to features of shape `(bs, c, h, w)` without changing dimensions, allowing the model to perform complex reasoning about global image structure and long-range dependencies before beginning the reconstruction process.
...
#
# **Your answer**: As the guidance scale increases, the generated digits become sharper and more recognizable, but also more stereotypical and less diverse. Higher guidance scales amplify the conditional signal relative to the unconditional one, pushing the model to generate images that more strongly conform to the expected characteristics of each digit class, trading off diversity for adherence to class-specific features.
